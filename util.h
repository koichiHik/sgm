...
__inline__ __device__ int __emulated_shfl(const int scalarValue,
...
ld_gbl_ca(const __restrict__ uint32_t *addr) {
...
__device__ __forceinline__ uint32_t
ld_gbl_cs(const __restrict__ uint32_t *addr) {
  uint32_t return_value;
  asm("ld.global.cs.u32 %0, [%1];" : "=r"(return_value) : "l"(addr));
  return return_value;
}

__device__ __forceinline__ void st_gbl_wt(const __restrict__ uint32_t *addr,
                                          const uint32_t value) {
  asm("st.global.wt.u32 [%0], %1;" ::"l"(addr), "r"(value));
}

__device__ __forceinline__ void st_gbl_cs(const __restrict__ uint32_t *addr,
                                          const uint32_t value) {
...
__device__ __forceinline__ void uint32_to_uchars(const uint32_t s, int *u1,
                                                 int *u2, int *u3, int *u4) {
  //*u1 = s & 0xff;
...
__device__ __forceinline__ uint32_t uchars_to_uint32(int u1, int u2, int u3,
                                                     int u4) {
...
__device__ __forceinline__ unsigned int vcmpgeu4(unsigned int a,
                                                 unsigned int b) {
  unsigned int r, c;
  c = a - b;
...
__device__ __forceinline__ void print_uchars(const char *str,
                                             const uint32_t s) {
  int u1, u2, u3, u4;
...
__inline__ __device__ uint8_t minu8_index4(int *min_idx, const uint8_t val1,
                                           const int dis, const uint8_t val2,
                                           const int dis2, const uint8_t val3,
                                           const int dis3, const uint8_t val4,
                                           const int dis4) {
  int min_idx1 = dis;
  uint8_t min1 = val1;
  if (val1 > val2) {
...
__inline__ __device__ uint8_t minu8_index8(
    int *min_idx, const uint8_t val1, const int dis, const uint8_t val2,
    const int dis2, const uint8_t val3, const int dis3, const uint8_t val4,
    const int dis4, const uint8_t val5, const int dis5, const uint8_t val6,
    const int dis6, const uint8_t val7, const int dis7, const uint8_t val8,
    const int dis8) {
  int min_idx1, min_idx2;
  uint8_t minval1, minval2;

  minval1 =
      minu8_index4(&min_idx1, val1, dis, val2, dis2, val3, dis3, val4, dis4);
  minval2 =
      minu8_index4(&min_idx2, val5, dis5, val6, dis6, val7, dis7, val8, dis8);
...
__inline__ __device__ int blockReduceMin(int val) {
  static __shared__ int
      shared[WARP_SIZE];  // Shared mem for WARP_SIZE partial sums
...
__inline__ __device__ int blockReduceMinIndex(int val, int idx) {
  static __shared__ int
      shared_val[WARP_SIZE];  // Shared mem for WARP_SIZE partial mins
  static __shared__ int
      shared_idx[WARP_SIZE];  // Shared mem for WARP_SIZE indexes
  const int lane = threadIdx.x % WARP_SIZE;
  const int wid = threadIdx.x / WARP_SIZE;
...
__inline__ __device__ bool blockAny(bool local_condition) {
  ...
  // read from shared memory only if that warp existed
  local_condition =
      (threadIdx.x < blockDim.x / WARP_SIZE) ? conditions[lane] : false;
